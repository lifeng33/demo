...
return {width:$(p).width(),height:$(p).height()-30};
};
})(jQuery);
(function($){
var _11=null;
...
validator:function(create_time_end){
	 var create_time_start = "";
	 if($('#frist_service_begin_time').length == 1)
	        create_time_start = $('#frist_service_begin_time').datebox('getValue');
	 if($('#create_time_start').length == 1)
	        create_time_start = $('#create_time_start').datebox('getValue');

     if(create_time_end != null && create_time_end.length > 10) {
         if($('#frist_service_end_time').length == 1)
             create_time_end = $('#frist_service_end_time').datebox('getValue');
         if($('#create_time_end').length == 1)
             create_time_end = $('#create_time_end').datebox('getValue');
     }

	 if(create_time_end != null && create_time_end != "" && 
	            create_time_start != null && create_time_start != ""){ 
            var start = new Date(create_time_start.replace("-", "/").replace("-", "/"));
		    var end = new Date(create_time_end.replace("-", "/").replace("-", "/"));
		    if (end < start) {
		        return false;
		    }
    }
...
validator:function(value){//由5-25个字符（包括小写字母、数字、下划线、中文）组成，一个汉字为两个字符，且不能全为数字
    var cArr = value.match(/[\u4e00-\u9fa5]/g);
    var cLength = value.length + (cArr == null ? 0 : cArr.length);
    if( cLength < 5 || cLength > 25 ){ //5-25个字符,一个汉字为两个字符
       return false;
   }
   if( /^[0-9]+$/.test(value) ){//不能全为数字
       return false;
   }
...
if(col){
// wqm var _6cd=_6ca[_6cc];
var _6cd = eval("_6ca['" + _6cc.replace(/\./g, "']['") + "']");
...
cc.push(">");
if(col.checkbox){
cc.push("<input type=\"checkbox\" "+(_6ca.checked?"checked=\"checked\"":""));
cc.push(" name=\""+_6cc+"\" value=\""+(_6cd!=undefined?_6cd:"")+"\">");
}else{
if(col.formatter){
